...
sudo port -v install \ gcc47 gcc48 \ mpich-default \ hdf5 +mpich
    ...
    py27-ipython +notebook +pyside +parallel \ py27-pyproj \
    py27-scipy py27-shapely \ py27-cython py27-pyflakes \ nco \ swig
    swig-python \ py27-netcdf4 \ xinit doxygen graphviz \
    py27-matplotlib \ gdal +mpich+spatialite+expat+geos+netcdf+python27 \
    ...
    +python27 +qt4 \ py27-scikit-image \ py27-virtualenv
...
sudo port -v install cppunit metis +openmp +gfortran superlu_dist +mpich sundials mumps +mpich +gfortran  petsc +parmetis +gfortran +mumps   boost +mpich eigen3 scotch dolfin +parmetis +scotch +petsc
